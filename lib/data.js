...
export const generateSlugs = async (location = PATH) => {
  ...
  for (let file of files) {
    ...
    else if (!file.isDirectory) res.push(file.name.replace(/\.mdx?$/, ''))
    else
      res.push(
  ...
  for (let file of files) {
    if (file.name === 'README.mdx') continue
      ...
      const meta = await getMetadata(
        path.join(location, file.name, 'README.mdx')
      )
      res.push({
        name: file.name,
        title: meta.title,
        // Optional folder README fields (see content/letters/README.mdx):
        icon: meta.icon || null, // @hackclub/icons glyph name
        iconColor: meta.iconColor || 'blue', // background color of the icon box
        collapsable: meta.collapsable || false, // if true, section starts collapsed
        kind: 'folder',
